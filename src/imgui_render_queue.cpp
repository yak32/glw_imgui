...
RenderQueue::RenderQueue() : m_size(0), m_mem_size(0), m_ready_to_render(false){
	m_render_options = 0;
	m_alpha = 255;
...
void RenderQueue::on_frame_finished() {
	if (m_size != GFXCMD_QUEUE_SIZE) {
		m_mem_size = m_size;
		m_size = GFXCMD_QUEUE_SIZE; // prevent queue update	during next frame
